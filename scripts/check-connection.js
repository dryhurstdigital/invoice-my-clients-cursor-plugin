...
if (!apiKey.startsWith("imc_live_") && !apiKey.startsWith("imc_dev_")) {
  console.warn(
    "[Invoice My Clients] Warning: IMC_API_KEY does not have the expected prefix.\n" +
    "API keys should start with 'imc_live_' (production) or 'imc_dev_' (development)."
  );
  process.exit(1);
}

console.log("[Invoice My Clients] API key configured. Ready to use invoice tools.");
process.exit(0);
